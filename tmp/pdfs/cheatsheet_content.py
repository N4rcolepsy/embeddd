...
def card(title, text=None, bullets=None, rows=None, code=None,
         sid=None, mode=None, file=None, needs=None):
    return dict(title=title, text=text, bullets=bullets, rows=rows,
                code=dedent(code).strip() if code else None,
                sid=sid, mode=mode, file=file, needs=needs)

def page(title, subtitle, left, right, sources):
    return dict(title=title, subtitle=subtitle,
                columns=[left, right], sources=sources)

PAGES = [
page("개발 경로 한눈에", "01 / START HERE  ·  먼저 ID 하나를 읽고, 그 경로를 확장한다.", [
    card("처음 2~3시간", rows=[
    ...
    card("책상 앞 구현 순서", text="자료 확보 → ID → 최소 설정 → raw → decode → 단위 변환 → 호출 주기 → 실패 처리. 각 단계의 성공 근거를 남긴 뒤 다음 단계로 간다.",
         bullets=["센서가 바뀌면? 장치 규칙을 수정한다.", "MCU가 바뀌면? 어댑터·보드를 수정한다.", "HAL_OK와 유효한 센서 샘플은 같은 조건이 아니다."]),
    card("스니펫 표기", rows=[
        ["C17 / C++17", "포함 헤더와 전제를 지키면 재사용할 수 있는 코드"],
        ["HAL", "기존 STM32F4 HAL 프로젝트·보드 설정이 필요"],
        ["구조 예시", "프로젝트 함수·상태·프로토콜을 채워야 하는 조각"]]),
], [
    card("영상에서 다시 볼 위치", rows=[
    ...
    card("검증 범위와 원문 관계", text="00~06, 목차, 출처 문서의 내용을 주제별로 압축했다. 통합본은 같은 내용을 묶은 파일이므로 중복 수록하지 않았다.",
         bullets=["영상 직접 내용과 이후 확장 설계는 구분한다.", "현행 온도 기준은 1885 LSB@25°C. 영상의 1852와 구분한다.", "과거 영상과 현재 공개 master 코드는 완전히 같지 않다.", "이 자료는 교육용. 전체 C/C++ 빌드·실물 검증은 미수행."]),
], "00 · 01 · sources/README / Phil's Lab #30, ADXL355 Rev. D"),

page("핸들·설정·출력 계약", "02 / API DESIGN  ·  입력 설정과 동작 중인 객체를 구분한다.", [
    card("공통 결과 타입", code=r'''
        #include <stdint.h>
        #include <stddef.h>
        #include <stdbool.h>
    ...
    ''', sid="S01", mode="C17", file="01_status.h", needs="공통 타입. 각 드라이버의 정책에 맞춰 확장."),
    card("API의 기본 모양", code=r'''
    ...
    ''', sid="S02", mode="구조 예시", file="02_api_shape.h", needs="S01. 불투명 타입의 정의·생성 정책은 별도."),
    card("각 포인터의 목적", rows=[
    ...
    card("숫자 0으로 오류를 표현하지 않는다", text="0°C·0g·레지스터 값 0은 정상일 수 있다. 상태는 반환값으로, 데이터는 out 포인터로 전달한다. 성공을 확인한 뒤 out을 사용한다."),
], "02 §§3~5, 9 · 03 §§5~6 · 04 §§3~4, 6"),

page("함수 포인터 + context", "03 / DEPENDENCY INJECTION  ·  무엇을 읽을지와 어떻게 읽을지를 나눈다.", [
    card("함수 타입과 함수 포인터", code=r'''
    ...
    ''', sid="S04", mode="C17", file="04_reg_bus.h", needs="S01. 동기 전체 전송, 주소 폭 8비트인 인터페이스."),
    card("호출 순서", code=r'''
        uint8_t id = 0;
    ...
    ''', sid="S05", mode="구조 예시", file="05_bus_call.c", needs="S04. bus·reg_id는 유효하게 설정."),
    card("이 인터페이스의 약속", bullets=[
        "DRV_OK = 요청한 전체 바이트 전송 완료.",
        "반환 뒤 src/dst를 보관하지 않는 동기 callback.",
        "실패한 dst는 일부 변경되어 있을 수 있다.",
        "reg·주소 폭·최대 n·timeout 단위는 어댑터 계약."]),
], [
    card("context로 장치별 연결을 전달", code=r'''
        typedef struct {
            uint8_t bytes[256];
        } Fake;
    ...
    ''', sid="S06", mode="C17", file="06_fake_read.h", needs="S01. 읽기 전용 메모리 모델이며 실제 레지스터 부작용 모델은 아님."),
    card("같은 코드를 다른 context와 사용", code=r'''
    ...
    ''', sid="S07", mode="구조 예시", file="07_fake_bind.c", needs="S04 + S06. f의 수명 안에서만 bus 사용."),
    card("함수와 객체의 타입을 맞춘다", bullets=[
...
], "02 §11 · 03 §6 · 04 §5 / callback + context"),

page("데이터시트 → 구현 계획", "04 / REGISTER CONTRACT  ·  주소보다 접근 조건과 의미를 먼저 적는다.", [
    card("작업 시작 전에 채울 표", rows=[
    ...
    ''', sid="S08", mode="구조 예시", file="08_register_worksheet.txt", needs="실제 부품의 데이터시트로 각 항목을 채운다."),
    card("세 종류의 주소", rows=[
        ["addr7", "I2C 버스에서 어느 장치를 선택하는가"],
        ["reg", "그 장치 내부의 어느 레지스터인가"],
        ["pointer", "MCU RAM 또는 MMIO의 어느 객체인가"]]),
], [
    card("초기화는 순서가 있는 절차", text="연결 검사 → 전원/ready → ID → 필요한 reset → 설정 가능한 상태 → 설정 적용 → 의미 있는 검증 → 측정 시작 → 첫 샘플 조건 → READY.",
         bullets=["이 순서는 틀이다. ID/reset 순서도 부품에 따라 다르다.", "reset/start 같은 명령은 반복해도 같은 의미인지 확인한다.", "실패하면 READY로 표시하지 않는다. 부분 적용 가능성을 남긴다."]),
    card("설정 갱신의 순서", code=r'''
    ...
    ''', sid="S09", mode="구조 예시", file="09_config_sequence.txt", needs="각 단계의 실패를 검사. 실제 함수가 아닌 설계 순서."),
    card("requested와 applied를 구분", bullets=[
    ...
    card("무엇을 읽을지 모를 때", rows=[
        ["센서 의미", "센서 datasheet + register map"],
        ["MCU 동작 순서", "MCU reference manual + errata"],
        ["함수 인수·상태", "해당 SDK의 header + source"],
        ["코어·IRQ", "Arm/CMSIS 문서"]]),
], "01 §§3~10 · 03 §§2~4, 8~9, 17~18"),

page("비트 필드·접근 정책", "05 / BITS  ·  마스크 계산과 하드웨어 쓰기의 의미를 분리한다.", [
    card("일반 RW 필드의 순수 값 계산", code=r'''
    ...
    ''', sid="S10", mode="C17", file="10_fields.h", needs="stdint.h. shift<8, mask/encoding은 유효. 값 범위는 호출 전에 검증."),
    card("비트 3:1에 mode=5 넣기", code=r'''
    ...
    ''', sid="S11", mode="구조 예시", file="11_field_use.c", needs="S10. old는 이미 읽은 일반 RW 값. 가상 필드 예."),
    card("연산 읽기", rows=[
        ["x & mask", "선택 비트만 남김"],
        ["x | mask", "선택 비트를 1로 만듦"],
    ...
    card("W1C는 RMW를 사용하지 않는다", code=r'''
        ...
        st = bus.ops->write(
            bus.ctx, status_reg,
            &clear_mask, 1U, 20U);
    ''', sid="S12", mode="구조 예시", file="12_w1c_write.c", needs="S04. 실제 register별 W1C·예약 비트 규칙 확인."),
    card("RMW를 쓸 수 있는 조건", bullets=[
    ...
    card("bitfield는 wire format이 아니다", text="C bitfield의 배치·padding·접근 폭을 임의로 레지스터 맵과 같다고 가정하지 않는다. 통신 버퍼·MMIO에는 mask/shift와 장치별 접근 함수를 우선 사용한다."),
], "01 §10 · 03 §4 · 04 §§8~10 / reserved bits, RMW, W1C"),

page("I2C·SPI 어댑터", "06 / TRANSPORT  ·  주소 변환과 프레임 규칙을 한곳에 둔다.", [
    card("STM32F4 HAL I2C 읽기 어댑터", code=r'''
        typedef struct {
    ...
    ''', sid="S13", mode="HAL", file="13_i2c_port.h", needs="S01 + stm32f4xx_hal.h. 초기화된 버스, 8비트 reg, 동기 읽기."),
    card("복사 전에 확인", bullets=[
    ...
    card("I2C register read의 파형", text="START → Addr+W → Register → repeated START → Addr+R → Data → 마지막 NACK → STOP. 각 바이트의 ACK/NACK도 포함한다.",
         bullets=["Transmit(reg) 뒤 Receive() 두 호출이 같은 파형을 보장하지는 않는다.", "MemAddSize는 내부 주소 길이. 센서 데이터의 분해능이 아니다.", "addr7=0x1D와 reg=0x08, RAM buffer 주소는 서로 다르다."]),
    card("ADXL355 9바이트 읽기", code=r'''
        uint8_t raw[9];
    ...
    ''', sid="S14", mode="HAL", file="14_adxl355_read.c", needs="버스·센서가 준비된 후 호출. 주소는 ASEL 회로와 일치해야 함."),
    card("SPI 프레임 체크", rows=[
    ...
    card("시간 예산", text="9바이트 데이터 + 장치 주소 2회 + reg 1바이트 = 12×9 = 108클록. 400 kHz에서 약 270 μs, 100 kHz에서 약 1.08 ms. START/STOP·stretching·소프트웨어 지연은 추가된다."),
], "01 §§6, 8 · 03 §7 · 05 §6 / ST HAL I2C, NXP UM10204"),

page("HAL·LL 선택과 초기화", "07 / STM32  ·  전송 상태 머신을 누가 소유하는지 먼저 정한다.", [
    card("HAL과 LL의 차이", rows=[
    ...
    card("GPIO 하나로 비교", code=r'''
        ...
        GPIOA->BSRR = (1UL << 5);
    ''', sid="S15", mode="HAL", file="15_gpio_compare.c", needs="HAL + stm32f4xx_ll_gpio.h. 클록·출력 설정 후 한 방법 선택."),
    card("자주 혼동하는 경계", bullets=[
    ...
    card("RAM handle / 하드웨어 instance", code=r'''
    ...
    ''', sid="S16", mode="HAL", file="16_hal_handle.c", needs="설명 조각. 이것만으로 I2C 초기화가 끝나지 않음."),
    card("혼용과 성능의 기준", bullets=[
...
], "05 전체 / classic STM32F4 HAL·LL 기준, HAL2 계약과 구분"),

page("UART 초기화 템플릿", "08 / UART INIT  ·  uint8_t* 버퍼와 UART handle*을 구분한다.", [
    card("타입 있는 설정과 핸들", code=r'''
        typedef struct {
    ...
    ''', sid="S17", mode="HAL", file="17_uart_api.h", needs="전체 파일은 existing/uart_template.h. 8N1·TX/RX 고정."),
    card("UART_Init 본체", code=r'''
        HAL_StatusTypeDef UART_Init(
    ...
    ''', sid="S18", mode="HAL", file="18_uart_init.c", needs="S17. 기존 전체 템플릿과 중복 링크하지 말 것."),
], [
    card("호출부: 객체를 만들고 주소 전달", code=r'''
    ...
    ''', sid="S19", mode="HAL", file="19_uart_start.c", needs="S17+S18 또는 existing 템플릿. 최초 초기화용."),
    card("메모리와 설정의 관계", rows=[
...
], "06 §§1~5 · examples/uart_template.* / 8N1, classic STM32F4 HAL"),

page("UART 송신·가변 인자", "09 / SEND + VARARGS  ·  초기화 옵션은 구조체, 가변 인자는 별도 계약이다.", [
    card("바이트 단위 동기 송신", code=r'''
    ...
    ''', sid="S20", mode="HAL", file="20_uart_write.c", needs="준비된 8N1 handle. task/main 전용, 접근은 외부 직렬화."),
    card("문자열은 끝의 NUL을 제외", code=r'''
    ...
    ''', sid="S21", mode="HAL", file="21_uart_send.c", needs="S20 선언 + 초기화된 uart. 실패 시 전송 rollback은 없음."),
    card("길이의 단위를 고정", bullets=[
    ...
    card("선언의 ... = 가변 인자", code=r'''
        void f(int a, ...);  /* declaration */
        /* Calls supply actual values: */
        f(10);
        f(10, 20, 30);
    ''', sid="S22", mode="구조 예시", file="22_ellipsis.txt", needs="인자 개수 관점의 문법 예. f의 실제 계약·구현은 별도."),
    card("count개 int를 읽는 계약", code=r'''
        #include <stdarg.h>
        #include <stdio.h>
    ...
    ''', sid="S23", mode="C17", file="23_varargs.c", needs="count>=0, 실제 추가 인자는 count개의 int. stdout 연결 별도."),
    card("va_arg는 타입 변환기가 아니다", bullets=[
...
], "06 §§6~8 · 04 §12 / C17 stdarg, ST UART Size 계약"),

page("바이트 → raw → 물리량", "10 / DECODE  ·  전송·해독·변환·공개를 각각 확인한다.", [
    card("명시적인 endian 해독", code=r'''
    ...
    ''', sid="S24", mode="C17", file="24_endian.h", needs="stdint.h. b는 읽을 수 있는 2바이트."),
    card("상위 정렬된 signed 20비트", code=r'''
    ...
    ''', sid="S25", mode="C17", file="25_signed20.h", needs="stdint.h. b는 3바이트, 하위 nibble은 측정값이 아닌 형식."),
    card("빠른 경계값", rows=[
        ["00 00 00", "0"], ["00 00 10", "1"],
        ["7F FF F0", "524287"], ["80 00 00", "-524288"],
        ["FF FF F0", "-1"]]),
], [
    card("ADXL355: 명목 스케일", code=r'''
    ...
    ''', sid="S26", mode="C17", file="26_adxl355_units.h", needs="stdint.h. 현행 기준·±2g 설정일 때. 개별 calibration·범위 변경은 별도."),
    card("성공한 완성 샘플만 공개", code=r'''
        ...
        *out = next;
        return DRV_OK;
    ''', sid="S27", mode="구조 예시", file="27_commit_sample.c", needs="dev/out 검증 후. decode가 next 전체를 초기화한다는 계약."),
    card("값이 맞는지 확인할 때", bullets=[
...
], "01 §§11~14 · 02 §§8~9 · 03 §10 / ADXL355 Rev. D"),

page("시간 제한·오류·복구", "11 / FAIL PREDICTABLY  ·  실패 원인을 보존하고 다음 호출 조건을 정한다.", [
    card("unsigned tick 차이로 경과시간", code=r'''
    ...
    ''', sid="S28", mode="C17", file="28_timeout.h", needs="stdint.h + stdbool.h. 동일 단위·단조 tick, 한 전체 wrap 이전에 관측."),
    card("전체 작업 budget을 전달", code=r'''
    ...
    ''', sid="S29", mode="구조 예시", file="29_operation_budget.c", needs="S28. 각 step의 timeout 계약이 전체 budget에 부합해야 함."),
    card("시간이 실제로 흐르는가?", bullets=[
    ...
    card("복구 책임의 경계", text="버스 계층은 거래를 정리하고, 장치 계층은 설정·샘플 유효성을 정리한다. 응용은 재시도·격리·사용자 알림 정책을 정한다. 낮은 계층에서 무한 재시도하지 않는다."),
], "03 §§8~9, 11, 17~19 · 04 §§12, 14"),

page("ISR·DMA·RTOS 수명", "12 / ASYNC  ·  시작 성공과 완료 성공을 분리한다.", [
    card("최소 알림: coalescing flag", code=r'''
        #include <stdatomic.h>
        #include <stdbool.h>
    ...
    ''', sid="S30", mode="C17", file="30_pending_flag.c", needs="대상 compiler/ABI가 ISR atomic 사용을 허용해야 함. payload 없는 알림."),
    card("이 flag의 의미", bullets=[
    ...
    card("소유권을 끊김 없이", text="submit → in-flight → 완료/오류/취소 처리 → 하드웨어가 더 이상 접근하지 않음 확인 → 버퍼 반환. 취소 요청만으로 버퍼가 안전해지지 않는다."),
], [
    card("비동기 구현 점검표", rows=[
    ...
    card("객체 종료 순서", text="새 요청 중지 → IRQ/콜백 등록·접근 종료 → 진행 중 전송 완료/취소 → 실행 중 콜백 종료 → 객체·버퍼 파괴. RAII도 이 비동기 순서를 자동으로 만들어 주지는 않는다."),
], "02 §10 · 03 §§14~16 · 04 §§13~14, 16 / target-specific synchronization"),

page("C 포인터·버퍼 빠른 참조", "13 / C17 CORE  ·  주소·크기·폭·수명을 명시한다.", [
    card("포인터 선언을 읽는 순서", rows=[
        ["T *p", "T 객체의 주소를 보관"],
    ...
    ''', sid="S31", mode="구조 예시", file="31_array_size.c", needs="stdint.h + stddef.h. 선언·호출 위치의 차이를 보는 조각."),
    card("덧셈 overflow 없는 범위 검사", code=r'''
    ...
    ''', sid="S32", mode="C17", file="32_bounds.h", needs="stddef.h + stdbool.h. 실제 객체도 capacity만큼 존재해야 함."),
    card("메모리 복사", rows=[
        ["memcpy", "겹치지 않는 유효 영역 복사"],
        ["memmove", "겹칠 수 있는 유효 영역 복사"],
        ["memset", "바이트 채우기. int 값·C++ 생성자 대체 아님"]]),
], [
    card("정수 연산 전에 폭을 정한다", code=r'''
        uint32_t word =
            ((uint32_t)hi << 8) | lo;
    ...
    ''', sid="S33", mode="구조 예시", file="33_integer_width.c", needs="stdint.h. raw*scale은 int64_t 안, hi/lo는 바이트라는 계약."),
    card("정수·부동소수점 체크", bullets=[
    ...
    card("포인터 cast로 프레임을 해독하지 않는다", text="*(uint16_t*)bytes는 정렬·별칭 규칙·CPU endian에 의존한다. wire format은 명시적 byte 조립으로 해석한다. packed도 이런 문제를 모두 해결하지 않는다."),
    card("const / volatile / restrict", rows=[
        ["const", "이 경로에서 수정하지 않음. 다른 경로의 변경까지 막지는 않음."],
        ["volatile", "주로 MMIO 접근. 동시성·순서 전체·DMA cache 보장 아님."],
        ["restrict", "특정 접근의 비별칭 계약. 확신할 때만 사용."]]),
    card("수명 확인", text="반환한 지역 변수 주소, 끝난 config/context 주소, 진행 중 DMA의 지역 배열을 보관하지 않는다. static은 수명을 늘리지만 재진입·공유 문제를 따로 만든다."),
], "04 §§3, 6~9, 13 · 06 §2 / C17 declarations and object lifetime"),

page("C 구조체·링키지·제어 흐름", "14 / C17 STRUCTURE  ·  모듈 경계와 데이터 표현을 분명하게 만든다.", [
    card("태그를 가진 union", code=r'''
        typedef enum {
    ...
    ''', sid="S34", mode="C17", file="34_event.h", needs="stdint.h. C와 C++의 비활성 union 멤버 규칙을 혼동하지 말 것."),
    card("구조체는 저장 형식과 다를 수 있다", bullets=[
    ...
    card("switch로 유한 상태를 드러내기", code=r'''
        switch (state) {
        case READY:
            return perform_read();
        case STARTING:
    ...
    ''', sid="S35", mode="구조 예시", file="35_state_switch.c", needs="S01. state·함수·상태 enum은 프로젝트에서 정의."),
], [
    card("C/C++ 공용 헤더 틀", code=r'''
        #ifndef DEVICE_API_H
        #define DEVICE_API_H
        ...
        #endif
        #endif
    ''', sid="S36", mode="C17", file="36_public_api.h", needs="C로 컴파일한 구현과 연결. C++ 클래스가 C로 변환되는 것은 아님."),
    card("이 키워드는 어디에 있는가?", rows=[
    ...
    ''', sid="S37", mode="C17", file="37_inline.h", needs="함수는 인자를 한 번 평가. SQUARE(i++) 같은 반복 평가 매크로와 구분."),
    card("오류 경로 정리", text="종속 단계는 실패 즉시 반환한다. 여러 자원의 해제가 필요하면 획득 순서 반대로 정리하는 단일 cleanup 경로도 유용하다. 정리 실패가 최초 오류를 덮어쓰지 않도록 한다."),
], "04 §§4, 10~12 / typedef, union, static, extern, header"),

page("C++17 드라이버 문법", "15 / C++  ·  객체 수명에 자원 책임을 연결하되 비동기 종료를 따로 설계한다.", [
    card("RAII: 잠금의 해제를 스코프에 연결", code=r'''
        struct Mutex;
    ...
    ''', sid="S38", mode="C++17", file="38_guard.hpp", needs="task 전용. lock은 획득 후 반환, unlock은 실패·예외 없음. 플랫폼 구현 필요."),
    card("반드시 읽을 표현", rows=[
    ...
    card("생성자 / init", text="생성자는 연결만 준비하고 실패할 수 있는 통신은 init()에서 상태 코드로 반환할 수 있다. 멤버 초기화 순서는 선언 순서다. class와 struct는 기본 접근 권한이 다르다."),
], [
    card("C callback에서 C++ 객체로", code=r'''
        class Reader {
        public:
    ...
    ''', sid="S39", mode="C++17", file="39_callback.cpp", needs="실제 callback typedef/ABI에 맞출 것. Reader 구현·등록 API 별도."),
    card("콜백과 이동의 수명", bullets=[
    ...
    card("완료 전 파괴하지 않는다", text="RAII가 자동으로 DMA·실행 중 callback을 취소하지 않는다. 명시적 stop/shutdown과 접근 종료를 보장한 뒤 자원을 파괴한다."),
], "04 §§15~18 / C++17 baseline, C++20 features explicitly marked"),

page("검증·찾아보기·원문 대응", "16 / FINISH  ·  하드웨어에서만 확인할 조건과 순수 계산을 구분한다.", [
    card("작은 성공의 증거를 남긴다", rows=[
    ...
    card("검증 범위", text="원문 measurement_decode의 Python 산술 모델은 20비트 전체 패턴 1,048,576개와 추가 reserved-bit·scale 사례를 통과했다. 이는 C 코드 실행 시험이 아니다. 이번 스니펫은 정적 검토 대상이며 C/C++ 빌드·보드 시험은 하지 않았다."),
], [
    card("모든 기존 Markdown의 대응", rows=[
...
], "Source review: 2026-08-31 / source-manifest.json contains file hashes")
]
